...
@login_required
def reports(request):
    return render(request, "auctions/reports.html")

@login_required
def csv_report(request):
    response = HttpResponse(content_type='text/csv')
    response["Content-Disposition"] = "attachment; report.csv"
    writer = csv.writer(response)
    report = Listing.objects.raw('SELECT *  FROM public.auctions_listing')
    writer.writerow(['Наименования', 'Описание', 'Ставки', 'Категория', 'Опубликовано'])

    for id in report:
        writer.writerow([id.productnames, id.descriptions, id.startingbids, id.category, id.created])

    return response

@login_required
def pdf_report(request):
    pdfmetrics.registerFont(TTFont('times', 'staticfiles/times.ttf'))
    ...
    report = Listing.objects.all()

    lines = []
    for id in report:
        lines.append(id.productnames)
        lines.append(id.descriptions)
        lines.append(str(id.startingbids))
        lines.append(id.category)
        lines.append(str(id.created))

    for line in lines:
        textob.textLine(str(Paragraph(line, styleN)))
